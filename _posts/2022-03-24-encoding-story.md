...
*For the sake of brevity and coherence, various detours, experiments, and failures have been ommitted from this story.*
...
I wasn't sure if sed would work, so first I tried to replace the `\xa0` byte with a bullet character to see if it works.
...
A quick internet search finds this solution for sed complaining about an illegal byte sequence:
...
It looks good. Similiarly, I used sed to replace invalid bytes in other files with an empty string. For example, for `\xad`:
...
One more invalid byte. After examining the line and searching the web for obvious character subtitutions, I decide to remove `0x8f` as well.
...
Finally. That's was exhausting. Time for a nap.
